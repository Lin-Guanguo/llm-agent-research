...
「Agent」这个词在 2026 年的中文互联网上,至少在指**三种完全不同的产品形态**。这三种产品形态服务不同的用户、解决不同的问题、用完全不同的技术栈搭建——但它们都被叫做「Agent 开发」。

**第一种是 Coding Agent**——以 Claude Code、Cursor、Manus 为代表的终端用户产品。一个开发者在终端里跟 LLM 说「帮我修这个 bug」,LLM 自己去读日志、跳到出错的函数、改代码、跑测试、提 PR,二十分钟交差。这一类产品的关键词是「**用**」——产品本身就是开盒即用的工具,你不需要再"搭"什么东西。

**第二种是 Agent 编排平台**——以扣子(Coze)、Dify 为代表的可视化低代码工具。一个产品经理在网页 UI 里拖二十几个节点(开始、知识库检索、LLM 调用、条件分支、消息发送),配好 prompt 和字段映射,花三天搭出一个内部 HR 政策咨询 bot,接到飞书机器人上。这一类的关键词是「**搭,但不写代码**」——目标用户是产品经理、运营、客服主管这种业务人员。

**第三种是 Agent 代码框架**——以 LangGraph、LangChain、Mastra 为代表的开发者库。一个 AI SaaS 创业者在 Python 项目里 import LangGraph,定义一堆 State,画一张 StateGraph,在每个节点里嵌一个带工具调用的 LLM,条件边根据模型输出路由到下一步,把整张图作为产品后端的一部分跑起来。这一类的关键词是「**搭,而且要写代码**」——目标用户是后端工程师。

---

这三种东西**凭什么都叫「Agent 开发」**?

这是 2026 年「Agent」这个词最大的问题。三个完全不同的产品形态、三个完全不同的用户群、三个完全不同的技术栈,挤在同一个名词下,所有人都觉得自己说的才是「真 Agent」。

这篇博客只做一件事:**把这三类 Agent 开发分清楚**——给每一类一个定义、几个主流代表、一段历史。看完之后,下次有人告诉你「我在做 Agent」,你能立刻判断他在做什么,而不是点头装懂。

---

## 二、为什么「Agent」这个词在 2026 年这么乱

要理解上面这三种东西怎么会挤在同一个词下面,先看一眼这个词的来历。

「Agent」不是 LLM 时代发明的。Russell & Norvig 那本经典 AI 教材——几乎所有计算机系学生都见过的那本砖头书——给的定义非常宽泛:**一个能感知环境、做决策、采取行动的系统**。温度计不算 agent(它不做决策),AlphaGo 算 agent(它决策)。这个定义 70 年代就有,一直用得挺好。

问题是 2022 年 LLM 开始爆发之后,这个词被重新拉出来用在 LLM 系统上——然后**彻底失控**。三个原因:

**名词跑得快,实现跟不上。** 2023 年 AutoGPT 红起来的时候,很多人第一次听到「AI Agent」这个词,默认 Agent = 能自动干活的 AI。但这个朴素理解一年内就被业界拉扯到要覆盖十种不同的东西——从在 IDE 里改一行代码,到在云端跑一晚上做数据分析,到在客服流程里回答一个 FAQ,全都叫 Agent。
...
**不同圈子对它的理解完全不同。** 学术圈说的 Agent 侧重「自主决策」,产品圈说的 Agent 侧重「能替用户干活」,开发者圈说的 Agent 侧重「框架抽象出 LLM + 工具的循环」。三个圈子用同一个词指不同的东西,然后还互相觉得对方在乱用。

---

我不尝试给 Agent 下一个权威定义——给词下定义是学术界的事,他们也没搞定。我做的是一件更朴素的事:**按产品形态分类**(也就是 §一 列的那三类:Coding Agent、Agent 编排平台、Agent 代码框架)。下面我们一类一类讲,每一类按历史顺序列主流代表。
