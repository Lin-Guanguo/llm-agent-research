...
### 个人观点：为什么通用 Agent 一定是 Coding Agent

我个人认为「通用 Agent = Coding Agent」不只是一个市场观察,而是一个必然。

**Coding 是计算机的原生语言。** 就像一个通用的人一定要会说英语(因为英语是当前世界的通用交流语言),一个真正通用的 Agent 一定要会写代码——因为代码是操作计算机最直接、最精确、最强大的方式。

你想让 Agent 帮你分析数据？它可以写一段 Python。你想让它帮你搭一个网站？它可以写 HTML + JS。你想让它帮你操作一个 API？它可以写 curl 或 requests。**只要 Agent 能写代码,它就可以通过代码去做几乎任何事。** 代码就是 Agent 的万能工具——不需要为每个场景预制一个专用 tool,写代码本身就是那个通用 tool。

这也解释了为什么 Claude Code / Manus 这类产品可以从 coding 场景自然扩展到通用任务,而反过来不行——**一个不会写代码的 Agent,本质上是一个被限制了表达能力的 Agent**。
...
先澄清一件事：**通用 Agent（§三）说的"Single-Agent"并不是说只有一个 LLM 在跑**。Claude Code 也会 spawn 子 agent 去做特定子任务——但关键是，这些 sub-agent 的结果是**汇报给主 agent**，由主 agent 在 ReAct 循环里决定下一步做什么。**主链路的控制权始终在一个 LLM 手里。**

企业/领域 Agent 也用多个 agent，但**谁在做调度**不一样：

- **通用 Agent**：**LLM 做协调者**——主 agent 在 ReAct 循环里自决一切，sub-agent 只是它的工具
- **企业 Agent**：**程序化 workflow 做协调者**——一个确定性的代码路径（或可视化 workflow）决定先跑哪个 agent、再跑哪个、条件分支怎么走

这是两类**最本质的架构差异**。不在于"有没有多个 agent"，而在于"**协调者是 LLM 还是代码**"。

企业场景选程序化 workflow 做协调者，是因为业务需要**可审计、可回滚、成本可预估**——这些东西让 LLM 自己协调天然给不了。
