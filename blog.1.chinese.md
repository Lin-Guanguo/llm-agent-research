...
如果 ReAct 循环这么简单，那 Coding Agent 的工程难度在哪里？

**答案是上下文管理**。LLM 的上下文窗口是有限的——一个长任务可能跑几十个循环，每个循环产生工具调用结果，这些结果累积起来很快就会塞爆上下文。

我读了六个 Coding Agent 产品的源码，它们各自发展出了不同的压缩策略：

- **Claude Code**：四层渐进压缩——从每轮静默截断，到缓存友好的部分清理，到完整的 9 段结构化摘要，到最后的激进裁剪。每一层只在前一层不够用时才升级
- **Codex CLI**：双路压缩——服务端加密压缩 + 客户端 LLM 压缩并行，per-item 工具输出截断（单个结果超 10KB 就截）
- **Gemini CLI**：验证式压缩——生成摘要之后再用一个 probe 问题验证摘要的质量，确保关键信息没丢

**上下文工程是 Coding Agent 的第一工程挑战，不是 ReAct 循环本身。** 循环很简单，但如何在几十步之后让 LLM 还能记得自己在干什么——这是真正的技术壁垒。

### 记忆：为什么都是纯文本文件

另一个有趣的工程选择：几乎所有 Coding Agent 的「记忆」都是纯文本文件。

- Claude Code → `CLAUDE.md`
- Codex → `AGENTS.md`
- Gemini CLI → `GEMINI.md`

没有数据库，没有向量检索，没有知识图谱。就是一个 markdown 文件，启动时整个加载到上下文里。

为什么这么做？因为**纯文本文件是 prompt cache 最友好的格式**。文件内容不变时，LLM 的 prompt cache 可以命中，每次调用的成本和延迟都大幅降低。一旦引入动态检索（比如向量搜索），cache 前缀就被破坏了。在 Claude Code 的架构里，**prompt cache 是一等公民**——每个设计决策都优先保护 cache 命中率。
