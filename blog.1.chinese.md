...
Status: Draft v1 (in progress, §一 done)

---

## 一、先看三个场景

下面三个人都是虚构的,但如果你身边有做 AI 的朋友,他们大概率能对上号。

---

**小林**,某互联网公司的产品经理。上周老板让她搭一个内部 HR 政策咨询机器人。她打开**扣子**,拖了二十多个节点:开始节点、知识库检索节点、LLM 节点、if-else 节点、消息节点……一个个配好 prompt 和字段映射,花了三天调通,接到飞书机器人上。她告诉我:「**我在做 Agent 开发**。」

**小张**,字节的后端工程师。下班回家开着 **Claude Code**,跟它说「这个 bug 你帮我修了」。它就自己去读日志、跳到出错的那个函数、改代码、跑测试、提 PR——二十分钟搞定,小张从头到尾没怎么敲键盘。他告诉我:「**我现在基本都用 Agent 写代码**。」

**老王**,一个 AI SaaS 创业者,产品是一个自动化知识库管家。后端用 **LangGraph** 写的:定义了一堆 State,画了一个 StateGraph,里面每个节点嵌一个带工具调用的 LLM,条件边根据模型输出路由到下一步。他告诉我:「**我在做 Agent 框架的开发**。」

---

小林、小张、老王都说自己在做 Agent。但你盯着这三句话看一会儿会发现——

**他们说的根本不是同一件事。**

- 小林在**用一个网站 UI 搭出一个业务 bot**——她点鼠标就能工作,一行代码都不用写
- 小张在**用一个命令行工具修代码**——他是工具的用户,Agent 替他干活
- 老王在**写 Python 代码调一个叫 LangGraph 的库**——他是开发者,用库搭建产品的后端

一个是可视化产品,一个是开发者工具,一个是代码框架。**它们凭什么都叫「Agent 开发」?**

这篇博客只做一件事:**把所有叫「Agent 开发」的东西分成清晰的三类**,给每一类一个定义、几个主流代表、一段历史。看完之后,下次有人告诉你「我在做 Agent」,你能立刻问对问题,而不是点头装懂。

---

## 二、为什么「Agent」这个词这么混乱

「Agent」这个词不是 LLM 时代发明的。你翻 Russell & Norvig 那本经典 AI 教材——就是几乎所有计算机系学生都见过的那本砖头书——会看到 Agent 的定义非常宽泛:**一个能感知环境、做决策、采取行动的系统**。温度计不算 agent(它不做决策),下象棋的电脑算 agent(它决策)。这个定义 70 年代就有,一直用得挺好。

问题是 2022 年 LLM 开始爆发之后,这个词被重新拉出来用在 LLM 系统上——然后**彻底失控**。

为什么会失控?三个原因:

**名词跑得快,实现跟不上。** 2023 年 AutoGPT 红起来的时候,很多人第一次听到「AI Agent」这个词,默认「Agent = 能自动干活的 AI」。但这个朴素的理解一年之内就被业界拉扯到要覆盖十种不同的东西——从在 IDE 里改一行代码,到在云端跑一晚上做数据分析,到在客服流程里回答一个 FAQ,全都叫 Agent。

**「Agent」听起来比别的词都潮。** 你的公司如果说「我们有一个 LLM 工作流」,投资人可能没什么感觉;如果说「我们有一个 AI Agent」,投资人立刻点头。于是所有人都开始把自己的东西叫 Agent,哪怕本质是一个 if-else 加一次 LLM 调用。词被滥用是因为它有溢价。

**不同圈子对这个词的理解完全不同。** 学术圈说的 Agent 侧重「自主决策」,产品圈说的 Agent 侧重「能替用户干活」,开发者圈说的 Agent 侧重「框架抽象出 LLM + 工具的循环」。三个圈子用同一个词指不同的东西,然后还互相觉得对方在乱用。

---

这篇博客**不尝试给 Agent 下一个权威定义**。给词下定义是学术界的事,而且他们也没搞定。我做的是一件更朴素的事:**按产品形态给「Agent 开发」分类**。判据简单到一句话:

> **你是在用它,还是在搭它?**

- 如果是「**用**」——它本身就是一个能直接使用的产品,你打开它就能干活。这是**第一类:Coding Agent**
- 如果是「**搭**」——再看搭的人是谁:
  - 非开发者在 UI 里拖节点——这是**第二类:Agent 编排平台**
  - 开发者在代码里写——这是**第三类:Agent 代码框架**

三类分完,几乎覆盖了市面上所有叫「Agent 开发」的事。下面一类一类讲,每一类按历史顺序列主流代表。
...
- **Cursor** 把 Agent 能力做进 IDE。开发者在编辑器里直接跟 Agent 说「把这个函数重构成 async」「帮我把这个 bug 修了」,Agent 在代码库里自己搜索、改文件、给出 diff。Cursor 成了 2024 年开发者付费最多的 AI 产品
...
**这是 Coding Agent 的范式胜利**。模型能力够了之后,架构反而回到了最朴素的循环。这件事我们在后面 §六 还会再讲一遍——它不是巧合,是有原因的。
...
这句话是国内最大的 agent 编排平台的产品团队,**公开承认他们主动绕开了 LLM 自主规划这个问题**。他们不是不知道 LLM 可以自己规划,他们是觉得「放开让 LLM 规划 = 不可控 = 业务不敢用」,所以选了 workflow 这条路。

这个承认很诚实,也很关键。它说明了整个第二类存在的合法性:**它不是技术上落后,是故意保守,为了换确定性**。这是企业落地 AI 应用的真实约束,不是技术路线的优劣。
...
LangChain 的第一个版本是 **2022 年 10 月 24 日**发布的,比 ReAct 论文晚 18 天。前面 §三 提过,作者 Harrison Chase 当时正在读 ReAct 论文,觉得这套循环应该被包装成开发者可以直接用的库,然后一周之内就写出来发到了 GitHub。

早期的 LangChain 里,agent 的抽象叫 `AgentExecutor` + **九种 `AgentType`**:`ZeroShotReact`、`StructuredChatReact`、`OpenAIFunctions`、`ReactDocstore`、`SelfAskWithSearch`……每一种都是围绕「ReAct 的某种 prompt 格式」展开的不同实现。本质上**全部都是 ReAct**,只是在 prompt 工程上做不同的包装。

这时候的 LangChain 是一个很单纯的东西:**一个把 ReAct 循环变成 Python API 的库**。

### 2023-05:LangChain 官方承认「单 ReAct 不够用」

AutoGPT 和 BabyAGI 爆发之后一个月——**2023 年 5 月 10 日**——LangChain 发了一篇官方博客,推出了一个叫 `plan_and_execute` 的新包。这篇博客里说了一句业界第一次**官方**承认 ReAct 局限性的话:

> 「Complex objectives + reliability → prompt size unsustainable」

翻译一下就是:任务一复杂,想让 LLM 可靠地完成,prompt 就会被塞爆,ReAct 循环撑不住。
...
**MetaGPT**(论文 2023 年 8 月)是华人研究者的论文 + 开源项目,把「多个 agent 扮演不同角色协作」做成了一个框架。它最有名的 demo 是让多个 agent 扮演「产品经理、架构师、工程师、QA」来完成一个软件项目。看 demo 视频很吸引人——每个 agent 都有自己的发言,互相评审对方的工作,最后输出一个完整的项目。

**AutoGen**(开源 2023 年 9 月)来自 Microsoft Research,思路类似:多个 agent 通过对话协作完成任务。
...
这段时间有一批新框架涌现。我就不一个个展开讲了,简单列一下:

- **Mastra** —— TypeScript 生态首选的 agent 框架
- **Google ADK** —— Google 官方的 Python Agent Development Kit
- **AgentScope** —— 阿里通义实验室开源(23k+ stars)
- **Pydantic AI** —— Pydantic 团队出品,用 Python 泛型和 schema 校验做类型最讲究的 agent 库
- **Vercel AI SDK** —— Vercel 的前端友好 SDK

这批框架的共同特征是:**外层是一个 workflow DAG(开发者定义),内层每个节点里跑一个 LLM ReAct 循环**。这个「workflow 外层 + ReAct 内层」的组合,在 2025 年成了**新一代代码框架的默认姿态**。不管它们用 TypeScript 还是 Python,底层是不是 LangGraph,这个架构已经是共识。
...
arxiv:2503.13657,*Why Do Multi-Agent LLM Systems Fail?*。这篇论文做了一件之前没人做过的事:**用 1600+ 条标注 trace 量化了 multi-agent 系统的失败模式**。

研究者先用 150 条 trace 构建了一个**包含 14 种失败模式**的 taxonomy,然后用 1600+ 条 trace 去验证。结果非常关键:**14 种失败模式里,有 8 种是「架构级」失败,不是「实现级」失败**。
...
抽象。但 Cognition 给了一个非常具象的例子,叫 **Flappy Bird 例子**。我直接讲:

> 假设你让一个 multi-agent 系统给你做一个 Flappy Bird 克隆。你有两个 subagent,一个负责做背景,一个负责做角色。
>
...
问题不是任何一个 subagent 错了。问题是**它们之间没有共享上下文,也没有共享中间决策**。每个 subagent 都做了自己合理的选择,但合在一起就是垃圾。
...
这个视角不是严格分类。**边界经常很模糊**——比如一个工作流平台里某个节点内部跑 ReAct,你说它算 Multi 还是 Single?Mastra 外层是 workflow DAG(像 Multi),内层每个节点跑单 ReAct(像 Single),它是哪边的?

但即使不严格,这个视角能帮你看清楚一个**清楚的趋势**:
...
- **小林**(用扣子搭客服 bot)→ 第二类:Agent 编排平台。她的产品形态是「人画 workflow + LLM 当节点」。她不需要懂代码,她需要懂业务流程
- **小张**(用 Claude Code 改代码)→ 第一类:Coding Agent。他不在「搭」什么,他直接用一个产品。他是 Coding Agent 这个产品的用户
- **老王**(用 LangGraph 写后端)→ 第三类:Agent 代码框架。他在用一个 Python 库写产品后端,他既是开发者也是用户的供应商

下次再有人跟你说「我在做 Agent」,你只需要问一个问题:
...
这篇博客做的是「盘点」——把 2026 年市面上的 Agent 开发分类讲清楚。

但有一个**灰色地带**这次没展开:某些生产场景里,Coding Agent 那种 LLM 完全自决的形态给不了**成本预估、审计追溯、合规控制**;编排平台的可视化 workflow 又太僵——加一个新业务 case 就得重画一张图;代码框架的默认 single-agent ReAct 也不完全对——因为它不给你**结构化的 Plan 产物**。

人们在这个灰色地带做了一些**介于三类之间**的东西。它们既不是 Coding Agent,也不是编排平台,也不完全是主流代码框架——它们是另外一种东西。

**下一篇**我聊这个灰色地带,以及为什么我自己做的生产 Agent 系统,在这个地带。
