...
5. **2025 年是一个重要转折**:Cognition 的《Don't Build Multi-Agents》和 Berkeley + Cornell 的 MAST 论文,从 production 经验和定量数据两个角度都质疑了「multi-agent 越拆越好」的乐观假设。这场辩论本身**还没有完全结束**(CrewAI 这类 role-based 框架依然有用户),但代码框架的设计天平在 2025 下半年明显往「单 agent + workflow 外层」一侧倾斜。LangChain 1.0(2025-10)连 `create_react_agent` 都 deprecated 了——**ReAct 赢麻了,连名字都不用挂了**
...
问题是 2022 年 LLM 爆发之后,这个词被重新拉出来用在 LLM 系统上——然后**含义逐渐变得模糊**。三个原因:
...
**AutoGPT**(2023-03-30)。作者 Significant Gravitas。它把 LLM 的输出结构化成一个 JSON,包含 `text / reasoning / plan / criticism / speak + command` 六个字段,每次让模型填一次,然后执行 command,再把结果喂回去循环。这是一种 **ReAct 风格** 的架构——LLM 在一个大循环里持续自决每一步。

**BabyAGI**(2023-04-01)。作者 Yohei Nakajima。它走的是另一条路线——**Plan-and-Execute**:有三个子 agent,一个是 `execution`(执行任务),一个是 `task_creation`(根据执行结果生成新任务),一个是 `prioritization`(对任务队列排序),再加上一个用 Pinecone 存的向量记忆。它的逻辑是先拆任务、再一步步执行。

**前后 3 天,两个爆款,代表两条不同的技术路线。**

这一段历史有一个反直觉的细节:很多人记得 2023 年的 Agent 狂潮是「一种东西」爆发起来的,但实际上从一开始就是两条路线并行——一条是 LLM 在一个大循环里自己决定下一步(AutoGPT 这条),另一条是先让 LLM 把任务拆成 todo list 再一步步执行(BabyAGI 这条)。这两条路线后来各自发展出了一整套框架生态。AutoGPT 这条线最后成了主流,BabyAGI 这条线则慢慢淡出了开源社区的中心位置。但这是后话。
...
### 个人观察:对这一档的长期未来其实有点怀疑

> 下面这段是我个人的看法,不是事实盘点。

我对 Agent 编排平台这一档的长期前景**其实不是很看好**——不是因为它现在不流行(它现在是国内 Agent 落地的事实标准),而是因为它的产品定位有一个长远的隐患。

Agent 编排平台**本质上是低代码平台**——只是这次的"代码"换成了「写一个调 LLM 的 workflow」。

低代码平台在传统编程领域已经有几十年历史了——OutSystems、Mendix、钉钉宜搭、各种 BPM 工具,每一代都试图把「写代码」这件事下放给非工程师。**但低代码平台一直有一个根本矛盾**:简单的需求用低代码足够,复杂的需求总会突破低代码的表达能力,然后用户被迫回到代码。这个矛盾决定了低代码平台只能在「足够简单且足够标准化」的场景里活下来。

Agent 编排平台基本是这套低代码逻辑套用到 agent 场景。但它现在面临一个新变量——**AI 写代码的能力本身在变强**。

如果有一天:

- 写代码的门槛因为 AI 而进一步降低(普通业务人员也能用 Coding Agent 让 AI 帮自己写一个 Python 后端服务)
- 代码框架的 API 因为 AI 而变得对业务人员可读(LangGraph 或 Mastra 直接由 AI 翻译成自然语言交互)

那这一档存在的核心理由——「让不写代码的人也能搭 agent」——可能就被消解了。**届时你可能不需要可视化 UI,你直接对 AI 说想要什么,AI 帮你写**。

当然这只是一种推演,要看几个变量:Coding Agent 写代码的可靠性能不能跨过那个门槛、企业愿不愿意把"AI 自己写的代码"放进生产、合规和审计在那种场景下怎么解决。所以这是一个**个人怀疑,不是预测**。

短期内(至少 2026-2027)第二类依然会蓬勃生长;但十年尺度上,它可能会面临比传统低代码平台更严峻的存在性挑战——因为它要竞争的不只是"开发者用代码"这条老对手,还有"AI 帮普通用户写代码"这个新对手。
...
### 2023-05:LangChain 官方提出「Plan-and-Execute」

AutoGPT 和 BabyAGI 爆发之后一个月——**2023 年 5 月 10 日**——LangChain 发了一篇官方博客,推出了一个叫 `plan_and_execute` 的新包。这是业界第一次**官方**讨论"在 ReAct 之外探索另一种 agent 架构"的可能性,博客里说:
...
翻译一下:当任务越来越复杂、越来越依赖 agent 跑生产时,prompt 里要塞的东西也越来越多(更多工具说明 + 更多历史上下文)——单纯的 ReAct 循环开始遇到 reliability 上的瓶颈。
...
**伏笔**:2025 年 Devin 的母公司 Cognition 发了 *Don't Build Multi-Agents*,加上 Berkeley + Cornell 发了 MAST 论文,从两个角度都开始质疑 role-based 多 agent 的可靠性。这个争论目前还没完全结束——CrewAI 现在还在用,有它的拥护者——但「拆 agent 越多越好」的乐观情绪已经明显降温。具体的论据我们留到 §六 展开讲。
...
整个博客最关键的一个时间点是 **2025 年**。这一年发生了三件事,加起来让代码框架的设计趋势开始转向。
...
arxiv:2503.13657,*Why Do Multi-Agent LLM Systems Fail?*。这篇论文做了一件之前没人系统做过的事:**用大规模的标注数据量化了 multi-agent 系统的失败模式**。研究者先用 150 条标注 trace 构建了一个**包含 14 种失败模式**的 taxonomy,然后用 1600+ 条 trace 去验证。一个关键发现是:**14 种失败模式里,有 8 种属于「架构级」**——也就是不一定能靠"换更聪明的模型"解决的那一类。

这是一个定量证据。它把"multi-agent 系统在生产环境为什么不稳"这个问题,从开发者的经验性观察,提升到了**有数据支撑的学术分析**。

**③ Cognition *Don't Build Multi-Agents*(2025 年 6 月 12 日)**

作者是 Cognition AI 的 Walden Yan——也就是 §三 提到的 Devin 那家公司。两年前发 Devin demo 把整个行业搅得天翻地覆的同一家公司,在 2025 年发了一篇文章,**反思自己在 multi-agent 上踩过的坑**,劝大家在搭 agent 时优先考虑单线程方案。
...
这三件事——Anthropic 的定义、MAST 的定量证据、Cognition 的实战反思——加起来,给业界提供了一个**有争议但越来越被认真对待的观点**:

> **「也许问题不只是『模型不够强所以需要更多脚手架』,而是『复杂度本身也有它的成本』。**
>
> **multi-agent 系统的不稳定不一定是模型能力问题,也可能来自架构本身——每次 handoff 都是一次隐式假设丢失的机会。」**

需要说明的是:**这个观点目前并没有成为定论**。CrewAI 这类 role-based 多 agent 框架依然有用户,有它的适用场景。但**代码框架的设计趋势确实在 2025 下半年明显转向**——大家从「往更复杂、更多 agent、更结构化的方向走」开始往回退,退到「单 agent + 优秀的上下文压缩 + workflow 做外层组织」这个更简朴的模式。

LangChain 1.0 在 2025 年 10 月发布时把 `create_react_agent` 都 deprecated,新名叫 `create_agent`,就是这个趋势在 API 层面的兑现。
...
这个视角不是严格分类。事实上,**Agent 和 Workflow 的边界本身在 2026 年正在变得越来越模糊**。

Anthropic 在 2024 年底给的「Workflow 是代码路径预定义的,Agent 是 LLM 动态决定流程」这个二分法,在 2026 年的实际产品里其实已经不那么干净了。看几个例子:

- **Mastra 这种代码框架**:外层是 workflow DAG(开发者画好流程,像 Workflow),内层每个节点跑单 ReAct(LLM 自决,像 Agent)。整体到底算 Workflow 还是 Agent?
- **扣子 / Dify 这种编排平台**:整体是工作流编排(像 Workflow),但每个 LLM 节点又允许 ReAct 风格的工具调用(像 Agent 嵌套在 Workflow 里)
- **Claude Code 这种 Coding Agent**:看上去是纯 Agent,但你给它一个 todo.md 文件让它按列表执行,它实际上就在按一个隐式的 workflow 跑

**Agent 和 Workflow 在 2026 年的实际产品里,越来越像一个连续光谱,而不是二分类**。"Multi 还是 Single"这个问题在很多情况下其实是问错了——更精确的问法是「这个产品的控制流主要靠人画,还是 LLM 自决,或者两者各占多少」。

但即使分类不精确,这个粗略视角依然能帮你看到一个**清晰的趋势**:

- 2022-2024 年,代码框架明显倾向于「拆」——不管是 role-based 多 agent、Plan-and-Execute、还是 multi-agent workflow,共同点都是「人工把任务分解」
- 2025 年起,Cognition 和 MAST 提供的两份证据让业界开始重新审视这个倾向。代码框架的设计天平开始往「单 agent ReAct + workflow 外层」一侧倾——但这个过程**还在进行中**,不是已经盖棺定论的事
- Coding Agent 和编排平台从头到尾都没在这场博弈里——它们各自服务不同的用户和需求,和这场技术路线的钟摆无关

一句话总结:**Coding Agent 是让 LLM 自己干,编排平台是给业务人员拆流程,代码框架是开发者在这两端之间做选择的工具箱。2025 年之后这个工具箱里的默认选项,从「多拆」开始往「少拆」一侧偏——但这是个进行中的趋势,不是定论**。
