...
具体到这张表，我想挑几个最戏剧性的对应讲一下——有些让我真的愣住过。

**LangGraph 的 checkpointer**。当我第一次看 LangGraph 怎么实现 "Agent 失败后从上一个节点恢复" 这个功能时，我看到它用 "channel + 快照" 的方式保存状态。我查了一下，LangGraph 官方文档里提到这个设计参考了 Pregel——Pregel 是 Google 2010 年的图计算系统，而 Pregel 的 checkpoint 机制直接继承自 Chandy-Lamport 1985 年的 snapshot 算法。**一个 2024 年发布的 LangChain 子项目，算法骨架是 40 年前论文**。

**Multi-agent hand-off**。在 LangGraph 或 CrewAI 里，一个 Agent "把任务交给另一个 Agent" 的机制是一等公民的设计。我后来发现这就是 **Carl Hewitt 在 1973 年提出的 Actor Model**——Actor 之间通过消息通信，一个 Actor 可以创建新 Actor、可以把任务委托给另一个。Erlang 在 1986 年把它工业化到电信系统。Akka 在 2010 年前后把它移植到 JVM。**这个模型在人类程序员手里火过又退潮了好几次，2024 年在 LLM 世界里重新活了过来**。

**Planner = Query Optimizer**。AgentScope 和一些研究性 Agent 框架里有"先出 Plan、再执行"的设计。Plan 是一棵任务树，里面带依赖关系。这就是数据库的**查询优化器**——SQL 先被解析成逻辑计划，再被优化成物理执行计划，带着 join 顺序和依赖。Goetz Graefe 在 1993-95 年的 Volcano/Cascades 框架已经把这套讲得非常透。**Agent 圈目前的 "Plan generation" 水平大概相当于 1985 年的查询优化器**——能跑、但没有 cost model、没有多方案比较。

**Prompt cache = CPU cache**。Anthropic 和 OpenAI 2024 年先后推出的 prompt caching 功能，本质就是**在 LLM 这个计算单元外面加一层带 TTL 的缓存**——热 prompt 命中 cache 就便宜。这和 1960 年代 IBM 给 CPU 加 L1 cache 解决的问题是同一个：**计算单元太贵，所以用一层廉价高速存储来摊销重复访问**。

这五个对应挑出来，每一个都足够写一篇自己的 blog。表里剩下的那些我就不一个个展开了——读者看到这里应该已经感觉到规律了。
...
这里我想引用一句话作为整篇文章的主轴金句：

> **Hardware changes, software principles don't.**

不管底下是打孔卡、晶体管、GPU 还是 LLM——只要它是一种"按规则把输入变成输出"的东西，你在它上面搭应用时就逃不开这四件事。

### 2.2 一个读者更熟悉的先例：Web 前端的 state 管理轮回

在直接跳到 Agent 之前，我想用一个**大多数工程师更熟悉的重演案例**来暖场——前端圈的 state 管理史。

前端从 2006 年 jQuery 开始，一路走到 Backbone MVC (2010)、Angular (2010)、Flux (2014)、Redux (2015)、MobX (2016)、Context API、Recoil、Zustand、Jotai……

表面看这是"前端框架在迭代"。**本质是同一个问题在反复重演**——"怎么管理 UI 的状态"。

而这个问题在计算机史上**早就解过无数遍**：
- Smalltalk-80 的 MVC 模式（1979）
- Microsoft MFC 的 Document/View 模型（1992）
- Java Swing 的 Observable + Observer（1998）
- WPF 的 data binding（2006）

Redux 自己的文档里大方承认它的思路来自 Elm 语言，而 Elm 直接继承自 90 年代的 Functional Reactive Programming。**Flux/Redux 不是发明，是把 FRP 重新搬到 DOM 这个新媒介上**。

前端圈这 20 年的重演之所以没人觉得"重复造轮子"，是因为每次新媒介（DOM、iOS UIKit、Android View、React Native、SwiftUI）出现，老答案都要被**重新翻译一遍**。不翻译过来的话，新媒介的使用者没法用。
...
### 2.4 重演的经济学——为什么不直接复用老轮子

这里有一个很自然的疑问：**既然老答案都在，为什么不直接用？为什么非要重新发明**？

有三个真实原因：

**1. 老轮子针对旧媒介优化，对新媒介不一定适用**

Flink 的 checkpoint 是为"每秒百万条短消息"设计的。barrier 算法假设单条消息处理时间在毫秒级。Agent 里一次 LLM 调用要 5 秒，中间还可能调外部 API，整个 checkpoint 周期是分钟级——Flink 那套延迟假设根本不成立。

同样的，Akka Actor 的 mailbox 是为微秒级消息设计的。Agent 之间传消息是 prompt，几百 KB 起步，速度是 Flink 消息的十万分之一——Akka 的很多优化在 Agent 场景里是纯粹的浪费。

**所以复用 ≠ 照搬**。老答案提供的是**思路**，不是**实现**。

**2. 老轮子的 API 对新手极不友好**

Akka 的 typed actor API 对没写过 actor 的人来说几乎无法上手。Flink 的 JobGraph 构造 API 一眼看过去全是 Java 模板代码。Erlang 的 OTP supervisor tree 概念要啃两个月才能用得动。

Agent 圈的从业者大多是从应用层上来的——Python 写得多，很少人写过 Scala、Erlang 或 Java Flink。**让他们直接用老工具，学习成本可能比重新造一个高**。

**3. Agent 圈的从业者大多不知道老轮子存在**

这是最现实的原因。一个 2024 年入行做 Agent 的工程师，极大概率没学过分布式系统论文、没读过 PL 经典、没写过流处理。他们遇到 "Agent 怎么从失败中恢复" 这个问题时，第一反应是自己想一个方案——而不是去搜 1985 年 Leslie Lamport 的论文。

**这件事一部分是遗憾，一部分是必然**。每一代工程师都会重新发明前人的东西，这是计算机文明的常态。但如果能意识到自己在重演，至少可以把重演做得**更快、更好、不踩同样的坑**。

### 2.5 从"重演"到"深挖"

到这里为止，我说的都是高层对应。但"高层相似 ≠ 本质同构"——如果只停在这一层，很容易被严肃读者正确地怀疑为"作者把所有相似都当成了同构"。

下一章要做的事是：**挑三个具体的对应关系，讲透它们在理论、工业化、Agent 应用三代之间的精确传承**。每一个都是我自己碰过的——一个是我写过一点、一个是我学过、一个是我现在正在做。
...
这张图是编译器 70 年的主要成就——**自动化了"已经被人结构化的源码"到"机器指令"的翻译**。
...
这导致 Agent 的 runtime 必须内置**retry、verify、fallback、replan** 这些东西作为一等公民——这些在传统 VM 里根本不存在，或者只是边角料。

这是第五章要展开的内容：**这是真正新的东西**。

**但核心观察不变**：LLM 是一种新型编译器，Agent 是它的配套 VM。Agent 工程师现在做的事，本质是**在给一种全新的"概率编译器"设计运行时**。这件事 70 年前没发生过。
...
- **国际象棋**：手工评估函数 → 被 Deep Blue 暴搜打败 → 被 AlphaZero 纯自对弈打败
- **围棋**：人类棋谱监督学习 → 被 AlphaZero 从零自学打败
- **计算机视觉**：手工特征 SIFT/HOG → 被 CNN 扫光
- **语音识别**：HMM + 手工音素规则 → 被端到端 seq2seq 扫光
- **NLP**：pipeline（parse → NER → coref → ...） → 被 LLM 扫光

**每次都一样**：人类工程师精心设计的结构，短期有效，长期被更大规模的纯学习扫光。

### 4.3 P&E / dataflow 是同类尝试

Plan-and-Execute、字段对齐、程序化校验、typed dataflow——这些 Agent 架构设计的本质是什么？

**是在约束 LLM 不要放飞**。

- 字段对齐：防止 LLM 乱起字段名
- 程序化校验：防止 LLM 输出垃圾数据
- Plan-and-Execute：防止 LLM 走到半路迷路
- Typed graph：防止 LLM 连错节点

这些全都是"**工程手段补救 LLM 能力不够**"。前提假设是——**LLM 会犯错，所以我要加一层护栏**。
...
**这是一种从未有过的"一次性软件"思维**。Flink 抽象是为了复用，Agent 图是为了不复用。前者是 Plato 的"理念"（可复用的抽象），后者是 Aristotle 的"具现"（每次具体的实例）。

70 年软件工程完全站在 Plato 那一侧。Agent 时代第一次出现了 Aristotle 那一侧的工程合理性。
...
把前三章的"重演"和这一章的"真正新的"合起来，可能的 framing 是：

> **LLM 不是颠覆，是补全 + 复活 + 再补全**。
>
> **补全**：编译栈最上层（意图→程序）被填上了，70 年来第一次。  
> **复活**：50 年休眠的 PL 遗产（dataflow、Actor、typed IR）被激活——因为新用户来了。  
> **再补全**：传统编译栈没处理过的"概率编译"问题，需要一整套新工程学。

前两件事是重演，第三件事是真新。这两者共存，才是 Agent 时代的完整图景。
...
每一条都有一个对应的 Agent 里的问题：

- Chandy-Lamport 对应**Agent 失败恢复**
- Flink backpressure 对应**Agent rate limiting 在多 agent 系统中的设计**
- Lee 的类型系统对应**Agent 节点接口的类型约束**
- React Fiber 对应**Agent replan 时怎么只重跑变化的子图**
- Programming Erlang 对应**multi-agent 通信模式**
- Volcano 对应**Agent Planner 的成本模型**（目前几乎没人做）
- Ptolemy 对应**Agent 图的整体设计模式**
- Unix OS 对应**Agent 系统的全局架构感**
...
*本文研究素材公开在 `/Users/linguanguo/dev/llm-agent-research/`，包括 20+ 个 Agent 框架的源码笔记、完整的对照表、以及这篇 blog 的讨论过程记录。*
